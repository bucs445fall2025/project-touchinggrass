# Sprint Meeting Notes

*note: replace anything surrounded by << >> and **remove** the << >>*

**Attended**: Luis Ramon, Stella Young, Rachel Todd

**DATE**: 11/02/2025 10pm

***

## Sprint 7 Review

### SRS Sections Updated

User interface

### User Story

- Piece_collision_user_story
- Website_user_story

### Sprint Requirements Attempted

- Implement tetris piece collision
- Embed tetris game on website

### Completed Requirements

- Implement tetris piece collision
- Embed tetris game on website

### The summary of the entire project

A tetris game you can play online on your computer (website friendly)

***

## Sprint 7 Planning

## Requirements Flex

4/5 requirement flexes remaining

### Requirement Target

- Further implement tetris piece collision
- Implement preview box for next tetris game piece

### User Stories

- Piece_collision_user_story
- Tetris_preview_box

### Action Items

- Further implement tetris piece collision
- Implement preview box to show the upcoming tetris piece for user
- Make sure game embedding is working on docker containerized website

### Issues and Risks

- Some blocks are working for piece collision but does not work for all the pieces which will worked on for this sprint

### Team Work Assignments

Further implement tetris piece collision and tetris preview piece box
